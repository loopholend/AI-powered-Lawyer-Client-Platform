...
public class LawyerRegisterServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (firstName == null || firstName.trim().isEmpty() ||
            ...
            response.sendRedirect("lawyer/lawyer-register.html?error=invalid");
            return;
        }
        ...
        try {
            ...
            if (rs.next()) {
                conn.rollback();
                response.sendRedirect("lawyer/lawyer-register.html?error=exists");
                return;
            }
            ...
            if (rsBar.next()) {
                ...
                pstmtBar.close();
                conn.rollback();
                response.sendRedirect("lawyer/lawyer-register.html?error=bar_exists");
                return;
            }
            rsBar.close();
            pstmtBar.close();
            ...
            if (rowsAffected > 0) {
                ...
                if (generatedKeys.next()) {
                    ...
                    conn.rollback();
                    response.sendRedirect("lawyer/lawyer-register.html?error=failed");
                }
            }
            ...
            e.printStackTrace();
            response.sendRedirect("lawyer/lawyer-register.html?error=database");
        } finally {
            try {
                if (rs != null) rs.close();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.sendRedirect("lawyer/lawyer-register.html");
    }
