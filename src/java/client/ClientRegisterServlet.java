...
public class ClientRegisterServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (firstName == null || firstName.trim().isEmpty() ||
            ...
            response.sendRedirect("client/client-register.html?error=invalid");
            return;
        }
        ...
        try {
            ...
            if (rs.next()) {
                conn.rollback();
                response.sendRedirect("client/client-register.html?error=exists");
                return;
            }
            ...
            if (rowsAffected > 0) {
                ...
                if (generatedKeys.next()) {
                    ...
                    conn.rollback();
                    response.sendRedirect("client/client-register.html?error=failed");
                }
            }
            ...
            e.printStackTrace();
            response.sendRedirect("client/client-register.html?error=database");
        } finally {
            try {
                if (rs != null) rs.close();
