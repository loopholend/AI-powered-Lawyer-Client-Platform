...
public class ChangePasswordServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String redirectPage = "client/client-profile.jsp";
        if ("lawyer".equals(userType)) {
            redirectPage = "lawyer/lawyer-profile.jsp";
        }
        ...
        try {
            ...
            if (!rs.next()) {
                response.sendRedirect(redirectPage + "?error=failed");
                return;
            }
            ...
            if (!storedHash.equals(currentPasswordHash)) {
                response.sendRedirect(redirectPage + "?error=wrongpassword");
                return;
            }
            ...
            if (rowsUpdated > 0) {
                response.sendRedirect(redirectPage + "?success=password");
            } else {
                response.sendRedirect(redirectPage + "?error=failed");
            }
